...
const selectClass = 'mt-2 min-h-12 w-full min-w-0 rounded-lg border border-outline-variant/60 bg-white px-3 text-sm text-on-surface focus-visible:outline-2 focus-visible:outline-primary'
  ...
  return <div className="space-y-5">
    <fieldset className="min-w-0">
      ...
      <div className="flex gap-2 overflow-x-auto px-1 py-1 md:flex-wrap">
        {[{ id: '', name: 'All exercises' }, ...metadata.categories.map((item) => ({ ...item, id: String(item.id) }))].map((item) => <button key={item.id} type="button" aria-pressed={query.category === item.id} onClick={() => onChange({ ...query, category: item.id })} className={`min-h-11 shrink-0 rounded-full border px-4 py-2 text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-primary ${query.category === item.id ? 'border-primary bg-primary text-white' : 'border-outline-variant/50 bg-white text-secondary hover:bg-surface-container-low'}`}>{item.name}</button>)}
      </div>
    </fieldset>
